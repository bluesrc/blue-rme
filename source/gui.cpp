...
void GUI::SetTitle(wxString title) {
	...
	if (title != "") {
		g_gui.root->SetTitle(title << " - Blue Map Editor BETA" << TITLE_APPEND);
	} else {
		g_gui.root->SetTitle(wxString("Blue Map Editor BETA") << TITLE_APPEND);
	}
#elif __SNAPSHOT__
	if (title != "") {
		g_gui.root->SetTitle(title << " - Blue Map Editor - SNAPSHOT" << TITLE_APPEND);
	} else {
		g_gui.root->SetTitle(wxString("Blue Map Editor - SNAPSHOT") << TITLE_APPEND);
	}
#else
	if (!title.empty()) {
		g_gui.root->SetTitle(title << " - Blue Map Editor" << TITLE_APPEND);
	} else {
		g_gui.root->SetTitle(wxString("Blue Map Editor") << TITLE_APPEND);
	}
#endif
}
